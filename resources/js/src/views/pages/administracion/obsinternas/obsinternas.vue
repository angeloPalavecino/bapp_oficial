      ...
      <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
        ...
        <div class="flex flex-wrap-reverse items-center">
          ...
          <vs-dropdown vs-trigger-click  class="cursor-pointer mr-4 mb-4">
            ...
            <vs-dropdown-menu >
              <vs-dropdown-item v-if="$can('obsinternas.destroy')" @click.prevent="$accion(1)">
                <span>Borrar</span>
              </vs-dropdown-item>
          ...
          <!-- ADD NEW -->
          <div v-if="$can('obsinternas.create')" class="p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-between text-lg font-medium text-base 
          text-primary border border-solid border-primary" @click="$agregarPopUp()" > <!-- @click="addNewDataSidebar = true" -->
              <feather-icon icon="PlusIcon" svgClasses="h-4 w-4" />
              <vx-tooltip color="primary" text="Agregar Obs. Interna">
              <span class="ml-2 text-base text-primary">Agregar Obs. Interna</span>
              </vx-tooltip>
          </div>
        </div>
        ...
        :active.sync="popupActive"   @close="$close($event)">
            ...
            <div class="flex flex-wrap items-center justify-center p-6" slot="footer">
              <vs-button v-if="modoEditar == true && $can('obsinternas.update')" class="mr-3" @click.prevent="$submitActualizar()" >ACTUALIZAR OBS. INTERNA</vs-button>
              <vs-button v-else-if="$can('obsinternas.store')" class="mr-3" @click.prevent="$submitAgregar()">AGREGAR OBS. INTERNA</vs-button>
            ...
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                   <div class="flex vx-col w-full sm:w-auto ml-auto mt-2 sm:mt-0">
                      ...
                      <vs-button v-if="$can('obsinternas.edit')" radius color="primary" type="border" icon-pack="feather" icon="icon-edit-2" size="small" class="ml-3" @click="editar(tr)"></vs-button>
                       </vx-tooltip>
                      <vx-tooltip color="primary" text="Eliminar">
                       <vs-button v-if="$can('obsinternas.destroy')" radius color="primary" type="border" icon-pack="feather" icon="icon-trash" size="small" class="ml-3" @click="$submitEliminar(tr, indextr)"></vs-button>
                       </vx-tooltip>
                 </div>
            </vs-td>
            </vs-tr>
...
export default {
  ...
  data() {
    return {
      ruta:'/observacionesinternas/observacionesinternas/',
      selected: [],
      items: [],
      itemsOriginal: [],
      itemsPerPage: 4,
