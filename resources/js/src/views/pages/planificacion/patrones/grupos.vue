      ...
      <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
        ...
        <div class="flex flex-wrap-reverse items-center">
          ...
          <vs-dropdown vs-trigger-click  class="cursor-pointer mr-4 mb-4">
            ...
            <vs-dropdown-menu >
              <vs-dropdown-item v-if="$can('grupopatrones.destroy')" @click.prevent="$accion(1)">
                <span>Borrar</span>
              </vs-dropdown-item>
          ...
          <!-- ADD NEW -->
          <div v-if="$can('grupopatrones.create')" class="p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-between text-lg font-medium text-base 
          text-primary border border-solid border-primary" @click="$agregarPopUp()" > <!-- @click="addNewDataSidebar = true" -->
              ...
              <span class="ml-2 text-base text-primary">Agregar Observacion</span>
              </vx-tooltip>
          </div>
        </div>

         <!-- POP UP -->
        <vs-popup class="holamundo"  ref="modal" :title="(modoEditar == false ? 'AGREGAR OBSERVACION' : 'ACTUALIZAR OBSERVACION')"  
        :active.sync="popupActive"   @close="$close($event)">
            <div class="p-5">
              <div>
                <vs-divider color="primary"><h5>Observaciones</h5></vs-divider>
              </div>
              <!-- DESCRIPCION -->
            ...
            <div class="flex flex-wrap items-center justify-center p-6" slot="footer">
              <vs-button v-if="modoEditar == true && $can('grupopatrones.update')" class="mr-3" @click.prevent="$submitActualizar()" >ACTUALIZAR OBSERVACION</vs-button>
              <vs-button v-else-if="$can('grupopatrones.store')" class="mr-3" @click.prevent="$submitAgregar()">AGREGAR OBSERVACION</vs-button>
      ...
      <template slot="thead">
        ...
        <template slot-scope="{data}">
            ...
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
              ...
              <vs-td>
                <p class="items-name">{{ tr.descripcion }}</p>
              </vs-td>
              <vs-td>
                   <div class="flex vx-col w-full sm:w-auto ml-auto mt-2 sm:mt-0">
                      <vx-tooltip color="primary" text="Editar">
                      <vs-button v-if="$can('grupopatrones.edit')" radius color="primary" type="border" icon-pack="feather" icon="icon-edit-2" size="small" class="ml-3" @click="editar(tr)"></vs-button>
                       </vx-tooltip>
                      <vx-tooltip color="primary" text="Eliminar">
                       <vs-button v-if="$can('grupopatrones.destroy')" radius color="primary" type="border" icon-pack="feather" icon="icon-trash" size="small" class="ml-3" @click="$submitEliminar(tr, indextr)"></vs-button>
                       </vx-tooltip>
                 </div>
            </vs-td>
            </vs-tr>
